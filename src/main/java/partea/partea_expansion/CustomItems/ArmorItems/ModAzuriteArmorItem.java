...
public class ModAzuriteArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, StatusEffectInstance> MATERIAL_STATUS_EFFECT_INSTANCE_MAP =
            (new ImmutableMap.Builder<ArmorMaterial,StatusEffectInstance>())
                    .put(ModArmorMaterial.DIAMOND_NETHERITE,new StatusEffectInstance(StatusEffects.REGENERATION,20,0,
                            false,false,false))
                    .build();
    private static final Map<ArmorMaterial, StatusEffectInstance> MATERIAL_STATUS_EFFECT_INSTANCE_MAP2 =
            (new ImmutableMap.Builder<ArmorMaterial,StatusEffectInstance>())
                    .put(ModArmorMaterial.DIAMOND_NETHERITE,new StatusEffectInstance(StatusEffects.RESISTANCE,20,0,
                            false,false,false))
                    .build();
    private static final Map<ArmorMaterial, StatusEffectInstance> MATERIAL_STATUS_EFFECT_INSTANCE_MAP3 =
            (new ImmutableMap.Builder<ArmorMaterial,StatusEffectInstance>())
                    .put(ModArmorMaterial.DIAMOND_NETHERITE,new StatusEffectInstance(StatusEffects.FIRE_RESISTANCE,20,0,
                            false,false,false))
                    .build();
    public ModAzuriteArmorItem(ArmorMaterial material, Type type, Settings settings) {
    ...
    private void evaluateArmorEffects(PlayerEntity player) {
        ...
        for (Map.Entry<ArmorMaterial,StatusEffectInstance> entry : MATERIAL_STATUS_EFFECT_INSTANCE_MAP3.entrySet()){
            ArmorMaterial armorMaterial = entry.getKey();
            StatusEffectInstance statusEffectInstance = entry.getValue();
