...
public class CrimsonPearl extends EnderEyeItem {
    private static TagKey<Structure> of() {
        return TagKey.of(RegistryKeys.STRUCTURE, new Identifier("nether_fossil"));
    }
    TagKey<Structure> NETHER_FOSSIL= of();
    public CrimsonPearl(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (blockHitResult.getType() == HitResult.Type.BLOCK && world.getBlockState(blockHitResult.getBlockPos()).isOf(Blocks.NETHER_BRICKS)) {
            return TypedActionResult.pass(itemStack);
        } else {
            user.setCurrentHand(hand);
            if (world instanceof ServerWorld) {
                ServerWorld serverWorld = (ServerWorld)world;
                BlockPos blockPos = serverWorld.locateStructure(NETHER_FOSSIL, user.getBlockPos(), 100, false);
                if (blockPos != null) {
